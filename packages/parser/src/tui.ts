...
import { getLatestCDSLink } from "./scrape.ts";
...
const saveResults = async (results: DownloadResult[]): Promise<string> => {
  ...
  const mapping: Record<string, { filename: string | null; url: string | null; status: string; year?: number }> = {};
...
// Print summary
const printSummary = (results: DownloadResult[]) => {
