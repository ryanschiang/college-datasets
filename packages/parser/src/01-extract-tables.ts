...
(async () => {
    ...
    contents: [
      {
        ...
        parts: [
          {
            text: "Extract the relevant table(s) from the document",
          },
        ],
      },
    ],
  ...
  await fs.writeFile(path.join(__dirname, "..", "example", `response-test.json`), JSON.stringify(responseJson, null, 2));
